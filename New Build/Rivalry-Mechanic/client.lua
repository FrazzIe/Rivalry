Mechanic:AddControls("flatbed", function(self, PlayerPosition)
	...
	if self.Control.Position ~= nil then
		Utilities:RenderMarker(25, self.Control.Position.x, self.Control.Position.y, self.Control.Position.z, self.Control.Direction, self.Control.Colour)

		if self.Help and TargetVehicle ~= 0 then
			Utilities:DrawEntityBoundingBox(TargetVehicle, 0, 255, 0, 255)
		end

		if #(PlayerPosition - self.Control.Position) < 1 then
...
Mechanic:AddControls("towtruck2", function(self, PlayerPosition)
	...
	if self.Control.Position ~= nil then
		Utilities:RenderMarker(25, self.Control.Position.x, self.Control.Position.y, self.Control.Position.z + 0.4, self.Control.Direction, self.Control.Colour)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for Model, Data in pairs(Mechanic.Vehicles) do
			Data.Vehicle = Utilities:GetClosestVehicle(PlayerPosition, 10, false, Data.Hash, PlayerVehicle)
