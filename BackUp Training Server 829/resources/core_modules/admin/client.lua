...
RegisterNetEvent("command:spawnp")
AddEventHandler("command:spawnp", function(_model)
		local model = GetHashKey(_model)
		RequestModel(model)
		while not HasModelLoaded(model) do
			Citizen.Wait(0)
		...
		local pos = GetEntityCoords(PlayerPedId(),false)
		local npc = CreatePed(26, model, pos.x, pos.y, pos.z, 0.0, true, true)
--[[		SetRelationShipBetweenGroups(4, model, GetHashKey("PLAYER"))
	...
	while true do
		...
		for k, v in pairs(SpawnedNpcs)do
			if(IsEntityDead(v))then
				SetRelationShipBetweenGroups(3, GetEntityModel(v), GetHashKey("PLAYER"))
				table.remove(SpawnedNpcs, npc)
			end
		end
	end
end)--]]

RegisterNetEvent("command:setmodel")
AddEventHandler("command:setmodel", function(_model)
		local model = GetHashKey(_model)
		RequestModel(model)
		while not HasModelLoaded(model) do
			Citizen.Wait(0)
