...
TriggerEvent("core:addGroupCommand", "setmodel", "staff", function(source, args, rawCommand, data, power, group)
    local source = source
    if not args[1] then
		TriggerClientEvent('chatMessage', source, "^0[^1SYSTEM^0]", {255, 0, 0}, "^3/setmodel [model]")
	else
		if args[1] then
			TriggerClientEvent("command:setmodel", source, args[1])
		else
			TriggerClientEvent('chatMessage', source, "^0[^1SYSTEM^0]", {255, 0, 0}, "^3/setmodel [model]")
		end
	end
end, {help = "Become any model you wish."})

TriggerEvent("core:addGroupCommand", "spawnp", "staff", function(source, args, rawCommand, data, power, group)
    local source = source
    if not args[1] then
		TriggerClientEvent('chatMessage', source, "^0[^1SYSTEM^0]", {255, 0, 0}, "^3/spawnp [model]")
	else
		if args[1] then
			TriggerClientEvent("command:spawnp", source, args[1])
		else
			TriggerClientEvent('chatMessage', source, "^0[^1SYSTEM^0]", {255, 0, 0}, "^3/spawnp [model]")
		end
	end
end, {help = "Spawn a Pedestrian."})