...
RegisterServerEvent("ems:submitreport")
AddEventHandler("ems:submitreport", function(r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13, r14, r15, r16, r17, r18)
    local source = source
    local report = exports['GHMattiMySQL']:QueryResult("INSERT INTO ems_reports ( `r1`, `r2`, `r3`, `r4`, `r5`, `r6`, `r7`, `r8`, `r9`, `r10`, `r11`, `r12`, `r13`, `r14`, `r15`, `r16`, `r17`, `r18` ) VALUES ( @r1, @r2, @r3, @r4, @r5, @r6, @r7, @r8, @r9, @r10, @r11, @r12, @r13, @r14, @r15, @r16, @r17, @r18 ); SELECT * FROM ems_reports WHERE `id` = (SELECT LAST_INSERT_ID());", { 
    ['@r1'] = r1,
    ['@r2'] = r2,
    ['@r3'] = r3,
    ['@r4'] = r4,
    ...
    TriggerClientEvent("ems:new-report", source, report)    
end)
...
RegisterServerEvent("ems:loadreports")
AddEventHandler("ems:loadreports", function(openUI)
    local source = source
    exports['GHMattiMySQL']:QueryResultAsync("SELECT * FROM ems_reports", {}, function(reports)
        if(reports[1] == nil) then
            TriggerClientEvent("ems:loadreportss", source, {}, openUI)
        else
            TriggerClientEvent("ems:loadreportss", source, reports, openUI)
        end
    end)
end)

RegisterServerEvent("ems:loadreport")
AddEventHandler("ems:loadreport", function(idv)
    local source = source
    local id = idv
    exports['GHMattiMySQL']:QueryResultAsync("SELECT * FROM ems_reports WHERE (id = @id)", {["@id"] = id}, function(report)
        TriggerClientEvent( "ems:loadreport", source, report[1] )
    end)
end)
...
function changeHealthStatus(character_id, source, status)
    local source = source
    healthstatus = exports['GHMattiMySQL']:QueryAsync("UPDATE emsmdt_database SET `health_status` = @health_status WHERE ( `character_id` = @character_id );", { 
        ['@health_status'] = status,
        ['@character_id'] = character_id,
    })
    TriggerClientEvent('ems:healthstatusupdate', source, status)
end

RegisterServerEvent('ems:mdtdoa')
AddEventHandler('ems:mdtdoa', function(firstname, lastname)
    local source = source
    local first_name, last_name = firstname, lastname
    exports['GHMattiMySQL']:QueryResultAsync("SELECT * FROM characters WHERE (first_name = @first_name) AND (last_name = @last_name)", {["@first_name"] = first_name, ["@last_name"] = last_name}, function(character)
        character_id = character[1].character_id
        exports['GHMattiMySQL']:QueryResultAsync("SELECT * FROM emsmdt_database WHERE (character_id = @character_id)", {["@character_id"] = character_id}, function(health)
            if(health[1].health_status == "Healthy")then
                changeHealthStatus(character_id, source, "Dead")
            else
                changeHealthStatus(character_id, source, "Healthy")
            end
        end)
    end)
end)