...
AddEventHandler("inventory:drop",function(data)
    ...
    if amount ~= nil then
        if math.floor(amount) > 0 then
            exports.ui:reset()
            exports.ui:open("inventory")
            TriggerServerEvent("inventory:drop", tonumber(data.item_id), data.name, math.floor(amount), FindNearbyPlayers(8), GetEntityCoords(PlayerPedId(), false))
        else
            TriggerEvent("inventory:open")
        end
    else
...
RegisterNetEvent("Inventory.Sync.Dropped.Items")
AddEventHandler("Inventory.Sync.Dropped.Items", function(Dropped)
    DroppedItems = Dropped
end)

RegisterNetEvent("Inventory.Pickup.Item")
AddEventHandler("Inventory.Pickup.Item", function(Item)
    addQty(Item.ItemID, Item.ItemQty)
    Notify("You have just picked up "..(Item.ItemQty).." "..(Item.ItemName).."!", 2500)
end)
