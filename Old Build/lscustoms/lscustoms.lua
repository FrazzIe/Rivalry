...
local function DriveInGarage()
	...
	if DoesEntityExist(veh) then
		...
		local repaircost = math.floor((1.5 * (((GetVehiclePetrolTankHealth(veh) * 0.1) + (GetVehicleBodyHealth(veh) * 0.5) + (GetVehicleEngineHealth(veh) * 0.5))/100)))
		LSCMenu:addPurchase("Repair vehicle", (repaircost > 800 and repaircost or 800), "Full body repair and engine service.")
		...
		Citizen.CreateThread(function()
	        ...
	        TriggerEvent("anticheat:set", "invincible", true, function(callback)
	            if callback then
	                ...
	                TriggerServerEvent("core:bug", "Line 500 in resource "..GetCurrentResourceName())
	                SetPlayerInvincible(GetPlayerIndex(),true)
	            end
	        end)
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
		...
		myveh.lplate = GetVehicleNumberPlateText(veh)
		myveh.engine_health = GetVehicleEngineHealth(veh)
		myveh.petrol_health = GetVehiclePetrolTankHealth(veh)
		myveh.vehicle_health = GetEntityHealth(veh)
		myveh.body_health = GetVehicleBodyHealth(veh)
