...
local atms = {
  ...
  {name="ATM", id=277, x = 526.90362548828, y = -160.65419006348, z = 57.084873199463},
  {name="ATM", id=277, x = 2682.8911132813, y = 3286.7404785156, z = 55.24112701416},
}
...
local banks = {
  ...
  {name="Bank", id=108, x=1175.1746826172, y=2706.4248046875, z=38.094032287598}
}
    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          DisplayHelpText("Press ~INPUT_CONTEXT~ to manage your ~y~bank account~w~!")
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            DisplayHelpText("~r~You can not use the ATM from a vehicle~w~.")
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You can not use an ATM by car");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    Notify("You must get closer")
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {255, 0, 0}, "^1You can not manage your money by car");
    else
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    TriggerEvent('chatMessage', "", {255, 0, 0}, "^1No near player");
  end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if (playing ~= false) then
    ...
    Notify("This player is not online")
  end
end)
