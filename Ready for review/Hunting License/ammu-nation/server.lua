...
Weapons = {
	...
	["WEAPON_COMPACTRIFLE"] = 120000,
	["WEAPON_ADVANCEDRIFLE"] = 1,
	["WEAPON_ASSAULTRIFLE"] = 1,
	["WEAPON_BULLPUPRIFLE"] = 150000,
...
RegisterServerEvent("hunters:givelicense")
AddEventHandler("hunters:givelicense", function(target)
	local source = source
	TriggerEvent("core:getuser", target, function(user)
		if user.get("hunters_license") == "false" then
			user.set("hunters_license", "true")
			TriggerClientEvent("hunters:set_license", target, "true")
			Notify("You gave a hunting license to "..user.get("first_name").." "..user.get("last_name"), 3500, source)
			Notify("You received your license to Hunt", 3500, target)
		else
			Notify(user.get("first_name").." "..user.get("last_name").." already has a hunting license!", 3500, source)
		end
	end)
end)
