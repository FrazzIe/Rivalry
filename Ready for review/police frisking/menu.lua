...
local charges = {
    ...
    {title="3rd Degree Felonies", items = {
        ...
        {charge="Felony Evading Causing Death/Injury (Vehicle)", time=10, cost=5000},
    ...
    {title="1st Degree Misdemeanors", items = {
        ...
        {charge="Fleeing/Eluding Law Enforcement (On Foot)", time=6, cost=2500},
...
AddEventHandler("police:menu_citizen_force", function()
    local t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
...
AddEventHandler("police:menu_citizen_frisk", function(type)
    exports.ui:open()
    local t, distance = GetClosestPlayer()
    if(distance ~= -1 and distance < 3) then
        TriggerServerEvent("police:frisk", GetPlayerServerId(t), type)
    else
        Notify("Please get closer to the target!", 2500)
    end
end)
