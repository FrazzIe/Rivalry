...
function math_round(num, numDecimalPlaces)
...
function IsMouseInBounds(X, Y, Width, Height)
	local MX, MY = math_round(GetControlNormal(0, 239) * 1920), math_round(GetControlNormal(0, 240) * 1080)
...
function GetSafeZoneBounds()
	...
	SafeSize = math_round(SafeSize, 2)
	SafeSize = (SafeSize * 100) - 90
	SafeSize = 10 - SafeSize

	local W, H = 1920, 1080

	return {X = math_round(SafeSize * ((W/H) * 5.4)), Y = math_round(SafeSize * 5.4)}
end
...
function UIMenuCheckboxItem:Selected(bool)
	if bool ~= nil then
		self.Base._Selected = to_bool(bool)
	else
...
function UIMenuCheckboxItem:Hovered(bool)
	if bool ~= nil then
		self.Base._Hovered = to_bool(bool)
	else
...
function UIMenuCheckboxItem:Enabled(bool)
	if bool ~= nil then
		self.Base._Enabled = to_bool(bool)
	else
...
function UIMenuProgressItem:CalculateProgress(CursorX)
	...
	self:Index(math_round(#self.Data.Items * (((Progress >= 0 and Progress <= self.Data.Max) and Progress or ((Progress < 0) and 0 or self.Data.Max))/self.Data.Max)))
end
...
function UIMenuGridPanel:CirclePosition(X, Y)
	if tonumber(X) and tonumber(Y) then
		...
		return math_round((self.Circle.X - (self.Grid.X + 20) + (self.Circle.Width/2))/(self.Grid.Width - 40), 2), math_round((self.Circle.Y - (self.Grid.Y + 20) + (self.Circle.Height/2))/(self.Grid.Height - 40), 2)
	end
end
			...
			if not self.Pressed then
				...
				Citizen.CreateThread(function()
					...
					while IsDisabledControlPressed(0, 24) and IsMouseInBounds(self.Grid.X + 20 + SafeZone.X, self.Grid.Y + 20 + SafeZone.Y, self.Grid.Width - 40, self.Grid.Height - 40) do
						Citizen.Wait(0)
						local CursorX, CursorY = math_round(GetControlNormal(0, 239) * 1920) - SafeZone.X - (self.Circle.Width/2), math_round(GetControlNormal(0, 240) * 1080) - SafeZone.Y - (self.Circle.Height/2)
				...
				Citizen.CreateThread(function()
					while IsDisabledControlPressed(0, 24) and IsMouseInBounds(self.Grid.X + 20 + SafeZone.X, self.Grid.Y + 20 + SafeZone.Y, self.Grid.Width - 40, self.Grid.Height - 40) do
						Citizen.Wait(75)
						local ResultX, ResultY = math_round((self.Circle.X - (self.Grid.X + 20) + (self.Circle.Width/2))/(self.Grid.Width - 40), 2), math_round((self.Circle.Y - (self.Grid.Y + 20) + (self.Circle.Height/2))/(self.Grid.Height - 40), 2)
...
function UIMenuPercentagePanel:Percentage(Value)
	if tonumber(Value) then
		...
		local Progress = (math_round(GetControlNormal(0, 239) * 1920) - SafeZone.X) - self.ActiveBar.X
		return math_round(((Progress >= 0 and Progress <= 413) and Progress or ((Progress < 0) and 0 or 413))/self.BackgroundBar.Width, 2)
	end
end
			...
			if not self.Pressed then
				...
				Citizen.CreateThread(function()
					while IsDisabledControlPressed(0, 24) and IsMouseInBounds(self.BackgroundBar.X + SafeZone.X, self.BackgroundBar.Y - 4 + SafeZone.Y, self.BackgroundBar.Width, self.BackgroundBar.Height + 8) do
						Citizen.Wait(75)
						local Progress = (math_round(GetControlNormal(0, 239) * 1920) - SafeZone.X) - self.ActiveBar.X
						self:UpdateParent(math_round(((Progress >= 0 and Progress <= 413) and Progress or ((Progress < 0) and 0 or 413))/self.BackgroundBar.Width, 2))
					end
				end)
			end
		end
...
function UIMenu.New(Title, Subtitle, X, Y, TxtDictionary, TxtName)
	...
	if Subtitle ~= "" and Subtitle ~= nil then
		...
		if string_starts(Subtitle, "~") then
...
function UIMenu:InstructionalButtons(bool)
	if bool ~= nil then
		self.Settings.InstrucitonalButtons = to_bool(bool)
	end
end
...
function UIMenu:MultilineFormat(str)
	if tostring(str) then
		...
		local output = ""
		local words = string_split(tostring(str), " ")
...
function UIMenu:DrawCalculations()
	...
	if self.Settings.MultilineFormats then
		if self.Subtitle.Rectangle and not self.Subtitle.Formatted then
			...
			local Linecount = #string_split(self.Subtitle.Text:Text(), "\n")
	...
	if #self.Items ~= 0 and self.Items[self:CurrentSelection()]:Description() ~= "" then
		...
		local Linecount = #string_split(self.Description.Text:Text(), "\n")
...
function UIMenu:Visible(bool)
	if bool ~= nil then
		self._Visible = to_bool(bool)
		self.JustOpened = to_bool(bool)
		self.Dirty = to_bool(bool)
		self:UpdateScaleform()
		if self.ParentMenu ~= nil or to_bool(bool) == false then
			return
		end
...
function UIMenu:ProcessControl()
	...
	if not self.UpPressed then
		if self.Controls.Up.Enabled and (IsDisabledControlJustPressed(0, 172) or IsDisabledControlJustPressed(1, 172) or IsDisabledControlJustPressed(2, 172) or IsDisabledControlJustPressed(0, 241) or IsDisabledControlJustPressed(1, 241) or IsDisabledControlJustPressed(2, 241)) then
			Citizen.CreateThread(function()
				self.UpPressed = true
				...
				self:UpdateScaleform()
				Citizen.Wait(175)
				while self.Controls.Up.Enabled and (IsDisabledControlPressed(0, 172) or IsDisabledControlPressed(1, 172) or IsDisabledControlPressed(2, 172) or IsDisabledControlPressed(0, 241) or IsDisabledControlPressed(1, 241) or IsDisabledControlPressed(2, 241)) do
					if #self.Items > self.Pagination.Total + 1 then
						self:GoUpOverflow()
					else
						self:GoUp()
...
function UIMenu:ProcessMouse()
	if not self._Visible or self.JustOpened or #self.Items == 0 or to_bool(Controller()) or not self.Settings.MouseControlsEnabled then
						...
						if Item:Selected() and Item:Enabled() then
							...
							elseif SubType == "UIMenuProgressItem" then
								if IsMouseInBounds(Item.Bar.X + SafeZone.X, Item.Bar.Y + SafeZone.Y - 12, Item.Data.Max, Item.Bar.Height + 24) then
									Item:CalculateProgress(math_round(GetControlNormal(0, 239) * 1920) - SafeZone.X)
									self.OnProgressChange(self, Item, Item.Data.Index)
									Item.OnProgressChanged(self, Item, Item.Data.Index)
								else
									self:SelectItem()
...
function MenuPool:MouseEdgeEnabled(bool)
	if bool ~= nil then
		for _, Menu in pairs(self.Menus) do
			Menu.Settings.MouseEdgeEnabled = to_bool(bool)
		end
	end
end
...
function MenuPool:ControlDisablingEnabled(bool)
	if bool ~= nil then
		for _, Menu in pairs(self.Menus) do
			Menu.Settings.ControlDisablingEnabled = to_bool(bool)
		end
	end
end
...
function MenuPool:ResetCursorOnOpen(bool)
	if bool ~= nil then
		for _, Menu in pairs(self.Menus) do
			Menu.Settings.ResetCursorOnOpen = to_bool(bool)
		end
	end
end
...
function MenuPool:MultilineFormats(bool)
	if bool ~= nil then
		for _, Menu in pairs(self.Menus) do
			Menu.Settings.MultilineFormats = to_bool(bool)
		end
	end
end
...
function MenuPool:DisableInstructionalButtons(bool)
	if bool ~= nil then
		for _, Menu in pairs(self.Menus) do
			Menu.Settings.InstructionalButtons = to_bool(bool)
		end
	end
end
...
function MenuPool:MouseControlsEnabled(bool)
	if bool ~= nil then
		for _, Menu in pairs(self.Menus) do
			Menu.Settings.MouseControlsEnabled = to_bool(bool)
		end
	end
end
