...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedSittingInAnyVehicle(Player) then
			local Vehicle = GetVehiclePedIsIn(Player, false)
			if not DecorGetBool(Vehicle, "hotwire") and GetPedInVehicleSeat(Vehicle, -1) == Player and not IsVehicleOwned(GetVehicleNumberPlateText(Vehicle)) and GetVehicleClass(Vehicle) ~= 18 and GetVehicleClass(Vehicle) ~= 13 and GetEntityModel(Vehicle) ~= GetHashKey("polmav") and GetEntityModel(Vehicle) ~= GetHashKey("defender") and GetEntityModel(Vehicle) ~= GetHashKey("Predator") and not keys_users[GetPlayerServerId(PlayerId())][GetVehicleNumberPlateText(Vehicle)] and string.sub(GetVehicleNumberPlateText(Vehicle), 1, 2) ~= "RR" then
