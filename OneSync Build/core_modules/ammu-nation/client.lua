...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsOnDutyPolice then
			for k,v in ipairs(ammu_nation.police) do
				local Distance = GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, true)
				if Distance < 15.0 then
					RenderMarker(25, v.x, v.y, v.z, 1.5, 1.5, 2.0, 255, 255, 0, 20)
					if Distance < 1.0 then
						DisplayHelpText("Press ~INPUT_CONTEXT~ to buy weapons!")
						if IsControlJustPressed(1, 51) then
							if not WarMenu.IsMenuOpened("Police_Weapons") then
								if not WarMenu.DoesMenuExist("Police_Weapons") then
									WarMenu.CreateMenu("Police_Weapons", "Armory")
									WarMenu.SetSpriteTitle("Police_Weapons", "shopui_title_gunclub")
									...
									WarMenu.OpenMenu("Police_Weapons")
								else
									WarMenu.OpenMenu("Police_Weapons")
								end
							else
						...
						if WarMenu.IsMenuOpened("Police_Weapons") then
							for k,v in pairs(PoliceWeapons) do
								local addCategory = true
								if v.License then
									if weapon_license ~= "true" then
							...
							if WarMenu.ComboBox("Armour", armourOptions, armourIdx, armourSelectedIdx, function(currentIndex, selectedIndex)
								...
								elseif armourSelectedIdx == 2 then
									SetPedArmour(PlayerPed, 0)
								end
							end
							if WarMenu.Button("Radio", "") then
								if not exports["rp-radio"]:IsRadioAvailable() then
									TriggerEvent("Radio.Set", true)
								else
									TriggerEvent("mythic_notify:client:SendAlert", { type = "error", text = "You already have a radio!" })
								end
							end
