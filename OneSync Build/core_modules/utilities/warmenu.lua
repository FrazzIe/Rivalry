    ...
    if menus[currentMenu] then
        ...
        if menus[currentMenu].sprite then
            if type(menus[currentMenu].sprite) == "table" then
                if #menus[currentMenu].sprite == 2 then
                    if not HasStreamedTextureDictLoaded(menus[currentMenu].sprite[1]) then
                        RequestStreamedTextureDict(menus[currentMenu].sprite[1], true)
                        while not HasStreamedTextureDictLoaded(menus[currentMenu].sprite[1]) do
                            Wait(0)
                        end
                    end
                    drawSprite(menus[currentMenu].sprite[1], menus[currentMenu].sprite[2], x, y, menuWidth, titleHeight, menus[currentMenu].spritecolour)
                else
                    if not HasStreamedTextureDictLoaded(menus[currentMenu].sprite[1]) then
                        RequestStreamedTextureDict(menus[currentMenu].sprite[1], true)
                        while not HasStreamedTextureDictLoaded(menus[currentMenu].sprite[1]) do
                    ...
                    drawText(menus[currentMenu].sprite[3], x, y - titleHeight / 2 + titleYOffset, menus[currentMenu].titleFont, menus[currentMenu].titleColor, titleScale, true)
                    drawSprite(menus[currentMenu].sprite[1], menus[currentMenu].sprite[2], x, y, menuWidth, titleHeight, menus[currentMenu].spritecolour)
                end
            else
                if not HasStreamedTextureDictLoaded(menus[currentMenu].sprite) then
                    RequestStreamedTextureDict(menus[currentMenu].sprite, true)
                    while not HasStreamedTextureDictLoaded(menus[currentMenu].sprite) do
                        Wait(0)
                    end
                end
                drawSprite(menus[currentMenu].sprite, menus[currentMenu].sprite, x, y, menuWidth, titleHeight, menus[currentMenu].spritecolour)
            end
        elseif menus[currentMenu].scaleform then
            local scaleform = RequestScaleformMovie(menus[currentMenu].scaleform)
            ...
            DrawScaleformMovie(scaleform, x+0.344, y+0.485, menuWidth+0.79, titleHeight+1.105, menus[currentMenu].scaleform_colour.r, menus[currentMenu].scaleform_colour.g, menus[currentMenu].scaleform_colour.b, menus[currentMenu].scaleform_colour.a)
        else
            drawRect(x, y, menuWidth, titleHeight, menus[currentMenu].titleBackgroundColor)
            drawText(menus[currentMenu].title, x, y - titleHeight / 2 + titleYOffset, menus[currentMenu].titleFont, menus[currentMenu].titleColor, titleScale, true)
        end
...
function WarMenu.CreateMenu(id, title)
    ...
    menus[id].spritecolour = {r = 255, g = 255, b = 255, a = 255}
    menus[id].scaleform = false
    menus[id].scaleform_colour = {r = 255, g = 255, b = 255, a = 255}
...
function WarMenu.CreateSubMenu(id, parent, subTitle)
    if menus[parent] then
        ...
        setMenuProperty(id, 'spritecolour', menus[parent].spritecolour)
        setMenuProperty(id, 'scaleform', menus[parent].scaleform)
        setMenuProperty(id, 'scaleform_colour', menus[parent].scaleform_colour)
...
function WarMenu.SetTitleFont(id, font)
    if type(font) ~= "number" then font = 0
    elseif font < 0 or font > 7 then font = 0 end
    setMenuProperty(id, 'titleFont', font)
end
...
function WarMenu.SetTitle(id, _title)
    setMenuProperty(id, 'title', _title)
end
function WarMenu.SetSpriteTitle(id, dicts, colour)
    setMenuProperty(id, 'sprite', dicts)
    setMenuProperty(id, 'spritecolour', colour or {r = 255, g = 255, b = 255, a = 255})
end

function WarMenu.SetScaleformTitle(id, dict)
    setMenuProperty(id, 'scaleform', dict)
end

function WarMenu.SetScaleformTitleColour(id, r, g, b, a)
    setMenuProperty(id, 'scaleform_colour', { r = r, g = g, b = b, a = a or menus[id].scaleform_colour.a })
end
