	...
	if type == "normal" or type == "upgrade" then
		...
		if vehicleHandle ~= nil and vehicleHandle ~= 0 then
			if type == "upgrade" then
				...
				SetVehicleWheelType(vehicleHandle, 7)
				for i = 0, 13 do
					SetVehicleMod(vehicleHandle, i, GetNumvehicleHandleMods(vehicleHandle, i) - 1, false)
				end
				SetVehicleMod(vehicleHandle, 14, 16, false)
				SetVehicleMod(vehicleHandle, 15, GetNumvehicleHandleMods(vehicleHandle, 15) - 2, false)
				SetVehicleMod(vehicleHandle, 16, GetNumvehicleHandleMods(vehicleHandle, 16) - 1, false)
				for i = 17, 22 do 
					TogglevehicleHandleMod(vehicleHandle, i, true)
				end
				SetVehicleMod(vehicleHandle, 23, 1, false)
				SetVehicleMod(vehicleHandle, 24, 1, false)
				SetVehicleMod(vehicleHandle, 25, GetNumvehicleHandleMods(vehicleHandle, 25) - 1, false)
				SetVehicleMod(vehicleHandle, 27, GetNumvehicleHandleMods(vehicleHandle, 27) - 1, false)
				SetVehicleMod(vehicleHandle, 28, GetNumvehicleHandleMods(vehicleHandle, 28) - 1, false)
				SetVehicleMod(vehicleHandle, 30, GetNumvehicleHandleMods(vehicleHandle, 30) - 1, false)
				SetVehicleMod(vehicleHandle, 33, GetNumvehicleHandleMods(vehicleHandle, 33) - 1, false)
				SetVehicleMod(vehicleHandle, 34, GetNumvehicleHandleMods(vehicleHandle, 34) - 1, false)
				SetVehicleMod(vehicleHandle, 35, GetNumvehicleHandleMods(vehicleHandle, 35) - 1, false)
				SetVehicleMod(vehicleHandle, 38, GetNumvehicleHandleMods(vehicleHandle, 38) - 1, true)
				SetVehicleTyreSmokeColor(vehicleHandle, 0, 0, 127)
				SetVehicleWindowTint(vehicleHandle, 1)
				SetVehicleTyresCanBurst(vehicleHandle, true)
				SetVehicleNumberPlateTextIndex(vehicleHandle, 5)
				SetVehicleModColor_1(vehicleHandle, 4, 12, 0)
				SetVehicleModColor_2(vehicleHandle, 4, 12)
				SetVehicleColours(vehicleHandle, 12, 12)
				SetVehicleExtraColours(vehicleHandle, 70, 141)
			end

			SetVehicleDirtLevel(vehicleHandle, 0)
			DecorSetBool(vehicleHandle, "hotwire", true)
			TaskWarpPedIntoVehicle(playerPed, vehicleHandle, -1)
		end
	end
end, false)
