...
local emplacement_garage = {
    [1] = {name="Public Garage", colour=1, sprite=357, x=-1189.1696777344,y=-687.54241943359,z=25.901292800903,id=1,gname="North Rockford Dr [1]",cost=3000,heading=39.566410064697,maxslots=5}, -- North Rockford Dr
    ...
    exports["GHMattiMySQL"]:QueryResultAsync("SELECT * from vehicles WHERE character_id=@character_id AND vehicles.escrowed = 0", {["@character_id"] = character_id}, function(vehicles)
        for i = 1, #vehicles do
            if os.time() > (tonumber(vehicles[i].lastpayment) + (604800 * 3)) and vehicles[i].payedoff > 0 and vehicles[i].weeks > 0 then
                ...
                if tonumber(vehicles[i].nextpayment) < os.time() and tonumber(vehicles[i].payedoff) ~= 0 then
                    if tonumber(vehicles[i].payedoff) <= 0 then
